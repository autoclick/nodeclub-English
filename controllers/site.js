...
//  Home caches work 。 Home is the need to take the initiative cache 
function indexCache() {
  var limit = config.list_topic_count;
  //  For all forum （tab） Do cache 
  [['', ' Whole ']].concat(config.tabs).forEach(function (pair) {
    //  Only the first page cache , page = 1。options  The reason why is because every generation  mongoose  Query ，
    //  Will change it 
...
setInterval(indexCache, 1000 * 5); //  Updated every five seconds 
indexCache();
// END  Home caches work 
  ...
  proxy.all('topics', 'tops', 'no_reply_topics', 'pages',
    function (topics, tops, no_reply_topics, pages) {
      res.render('index', {
        ...
        tabs: config.tabs,
        tab: tab,
        pageTitle: tabName && (tabName + ' Forum '),
      });
    });
};
